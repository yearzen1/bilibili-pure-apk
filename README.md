...
| 观看历史     | 分 P 进度显示、30 秒心跳上报、服务端 API 搜索（匹配标题 + UP主 名字）、回到顶部按钮              |
...
| 字幕         | 支持 B站 AI 字幕和 CC 字幕，可拖拽定位（上下左右），位置持久化                                   |
| 播放器状态栏 | 全屏播放顶部显示网络类型与电量                                                                   |
| 合集         | 视频详情页合集列表（BottomSheet 弹出、分页、排序）                                               |
| 评论增强     | GIF 动画、B站自定义表情内联、置顶/点赞/排序、图片查看器（缩放+原图保存）                         |
| 笔记查看     | 评论区识别笔记链接，应用内 WebView 渲染全文                                                      |
| 应用内更新   | GitHub 版本比对、下载安装、Markdown 更新日志                                                     |
...
| 字幕轨道       | `api.bilibili.com/x/player/wbi/v2`                           | 获取字幕轨道列表（AI/CC）                                                 | ✅   |
| 评论           | `api.bilibili.com/x/v2/reply/main`                           | 视频评论（游标分页）                                                      | ✅   |
| 回复           | `api.bilibili.com/x/v2/reply/reply`                          | 评论回复（最多 20 条）                                                    | ✅   |
| 评论操作       | `api.bilibili.com/x/v2/reply/action`                         | 点赞/踩评论（需 csrf）                                                    | ✅   |
...
| 历史搜索       | `api.bilibili.com/x/web-interface/history/search`            | 服务端搜索历史记录（匹配标题 + UP主 名字）                                | ✅   |
...
- **图片加载**: Coil 2.6.0（复用同一 OkHttp 客户端处理 CDN 鉴权）+ `coil-gif` 评论区 GIF 动画
- **图片查看**: Telephoto 0.12.1（评论图片缩放+原图保存）
...
通过 `x/player/wbi/v2` 获取字幕轨道列表，支持 B站 AI 字幕和 CC 字幕：

- `getPlayerInfo(aid, cid)` 返回 `subtitle.list`，包含字幕 URL（`subtitle_url` 为相对路径，需补 `https:`）
- JSON 格式：`body[]` 含 `from`（开始秒）、`to`（结束秒）、`content`（文本）
- `SubtitleParser` 使用二分查找定位当前 cue，支持多条字幕同时显示
- `SubtitleOverlay` 使用 `BoxWithConstraints` + `Modifier.offset` 渲染，`onGloballyPositioned` 动态计算边界
- 拖拽定位：本地 `MutableState` 1:1 跟踪手指，`LaunchedEffect` 同步 ViewModel + 文字变化时重新 clamp
- 位置通过 SharedPreferences 持久化（`subtitleOffsetX`/`subtitleOffsetY`）
...
通过服务端 API 实现搜索，匹配标题和 UP主 名字：

1. `BilibiliRepository.searchHistory(keyword)` 调用 `GET x/web-interface/history/search`
2. 返回 `HistorySearchItem`（字段：`title`/`cover`/`author_name`/`author_face`/`history.bvid` 等），与历史列表 API 结构不同
3. `HistoryViewModel.toHistoryItem()` 将 `HistorySearchItem` 映射为 `HistoryItem` 供 UI 使用
4. `HistoryScreen` 通过 `derivedStateOf` 切换 `displayItems`：搜索时取 `searchResults`，否则取 `items`
